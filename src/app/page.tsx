...
// Pricing Section
function Pricing() {
  // Set to true when running a discount promotion
  const hasPromo = true;
  const promoCode = 'LAUNCH25';
  const promoDiscount = 0.25; // 25% off
  ...
  return (
    <section id="pricing" className="py-24 md:py-32 relative">
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="text-center mb-16">
          {hasPromo && (
            <div className="inline-flex items-center gap-2 bg-green-500/20 border border-green-500/50 text-green-400 text-sm font-semibold px-4 py-2 rounded-full mb-6 animate-pulse">
              <span>🎉</span>
              25% OFF First Month — Use code <span className="font-bold">{promoCode}</span>
            </div>
          )}
          <h2 className="text-3xl md:text-5xl font-bold mb-4">
            ...
            >
              ...
              <div className="mb-6">
                ...
                <div className="flex items-baseline gap-2 mb-2 flex-wrap">
                  {hasPromo && (
                    <span className="text-2xl text-white/40 line-through">{plan.price}</span>
                  )}
                  <span className={`text-4xl font-bold ${hasPromo ? 'text-green-400' : ''}`}>
                    {hasPromo 
                      ? `$${(plan.originalPrice * (1 - promoDiscount)).toFixed(2)}`
                      : plan.price
                    }
                  </span>
                ...
                {hasPromo && (
                  <div className="text-xs text-green-400 mb-2">First month only • Then {plan.price}/mo</div>
                )}
